...
// Error types
typedef enum {
    UNDEFINED, MISPAREN, NOTNUMID, NOTFOUND, RUNOUT, NOTLVAL, DIVZERO, SYNTAXERR,fuck
} ErrorType;
...
int ck(BTNode *root){
    ...
    if(root->data==ID)
        flag=1;
    if(root->left) ck(root->left);
    if(root->right) ck(root->right);
    return flag;
}
...
void printAssembly(BTNode *root,int side) {
    static int ok=0;
    if(root->data==ASSIGN){
        if(root->right) printAssembly(root->right,0);
        ...
        if(root->left) printAssembly(root->left,side);
        if(root->right) printAssembly(root->right,side);
        if(root->data==ID){
            root->regis=cnt;
            for (int i = 0; i < sbcount; i++) {
                if (strcmp(root->lexeme,table[i].name) == 0) {
                    if(side==0&&table[i].fuck<=1) error(NOTFOUND);
                    root->regis=cnt;
                    printf("MOV r%d [%d]\n",cnt,i*4);
                    cnt++;
                }
